...
- критичні сценарії модуля перевіряються окремим MongoDB E2E-набором `npm run test:e2e:db -- electives.e2e-spec.ts`: RBAC, кафедральна ізоляція, конкурентний вибір останнього місця, ліміт виборів, безпечне скасування, фіналізація, інтеграція з курсами, аудит і CSV-захист.
